...
export function App({ today = new Date() }: Props = {}) {
  ...
  const [selectedId, setSelectedId] = useState<string | null>(() =>
    readStackId(window.location.hash),
  );
  const [comparing, setComparing] = useState(() =>
    isCompareHash(window.location.hash),
  );
  const [needing, setNeeding] = useState(() =>
    isNeedHash(window.location.hash),
  );
  ...
  useImportOnLoad(
    useCallback((imported: WoodStack[]) => replaceStacks(imported), []),
  );
  ...
  useEffect(() => {
    const newlyReady = findNewlyReady(
      stacks,
      (stack) =>
        getCachedNormals(stack.location.latitude, stack.location.longitude),
      today,
    );
        ...
        <Stack gap="lg">
          ...
          {comparing ? (
            ...
            <StackList
              ...
              normalsFor={(stack) =>
                getCachedNormals(
                  stack.location.latitude,
                  stack.location.longitude,
                )
              }
